...
def main():
    ...
    tf.io.write_graph(final_gd, "../pb/", "frozen_shape.pb", as_text=False)
    tf.io.write_graph(final_gd, "../pb/", "frozen_shape.pbtxt", as_text=True)
...
def fold(graph_def, bn_scope, conv_scope):
    ...
    epison_value = values_from_attr(nodes, f'{bn_scope}/cond/FusedBatchNormV3_1', 'epsilon')
    ...
    fused_bn_node = node_from_name(nodes, f'{bn_scope}/cond/FusedBatchNormV3_1')
    ...
    print('scale value shape is ', scale_value.shape)

    scaled_weights = np.copy(kernel_value)

    print('scale weight shape is ', scaled_weights.shape)
    ...
    while not it.finished:
        ...
        it[0] *= current_scale
        it.iternext()
        print('multi index is', it.multi_index[3])
    print('scale weight cnt is ', cnt)
